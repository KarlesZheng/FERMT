...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='FERMT', help='training script name')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python /home/hipeson/zjk/FERMT-main/lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d /home/user-njf87/zjk/FERMT-tiny4/lib/train/run_training.py " \
