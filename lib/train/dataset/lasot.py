...
class Lasot(BaseVideoDataset):
    ...
    def _read_nlp_anno(self, seq_path):
        ...
        # phrase = pandas.read_csv(nlp_anno_file, delimiter=',', header=None, dtype=str, na_filter=False, low_memory=False).values
        with open(nlp_anno_file, 'r') as f:
            phrase = f.read()
        return phrase
    ...
    def get_sequence_info(self, seq_id):
        ...
        phrase = self._read_nlp_anno(seq_path)

        valid = (bbox[:, 2] > 0) & (bbox[:, 3] > 0)
        visible = self._read_target_visible(seq_path) & valid.byte()

        return {'bbox': bbox, 'valid': valid, 'visible': visible, 'phrase':phrase}
